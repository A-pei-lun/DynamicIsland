...
namespace DynamicIsland.Island
{
    ...
    /// </summary>
    /// <summary>alert 呈现方式。Summary=轻摘要非抢占（保持当前尺寸只换内容，无动作按钮）；
    /// Interactive=重交互（现有 InlineAlertPanel，会生长 + 动作按钮）。默认 Interactive。</summary>
    public enum AlertKind { Interactive, Summary }
    ...
    public interface IIslandAlert
    {
        ...
        /// <summary>呈现方式。Summary=轻摘要非抢占（保持当前尺寸只换内容，无动作按钮）；
        /// Interactive=现有 InlineAlertPanel（生长 + 动作按钮）。默认 Interactive。</summary>
        AlertKind Kind => AlertKind.Interactive;
    }

    /// <summary>
    /// 提醒的动作按钮：一个标签 + 一个回调。alert 携带它即表示胶囊右侧可点出一个动作按钮。
    /// 回调执行后由 MainWindow 内联面板关闭当前提醒。
    /// </summary>
