...
```bash
# 克隆
git clone https://github.com/APIRL/DynamicIsland.git
cd DynamicIsland
...
```bash
# 前提：安装 Inno Setup 6 (https://jrsoftware.org/isdl.php)
# 或直接运行发布脚本（自动检测 ISCC）
powershell -ExecutionPolicy Bypass -File build-release.ps1
```

运行后生成 `publish/DynamicIsland-Setup-1.0.0.exe`，双击安装，自带卸载入口（控制面板「添加或删除程序」/ 开始菜单）。
...
将 `publish/` 整个目录分发到任意路径，运行 `DynamicIsland.exe` 即可。

### 卸载

- **安装器安装**：控制面板 → 添加或删除程序 → DynamicIsland → 卸载
- **手动安装**：直接删除整个目录，无残留注册表
