...
namespace DynamicIslandPro
{
    public partial class MainWindow : Window
    {
        ...
        {
            InitializeComponent();
            _glass = new LiquidGlassRenderer(this, GlassCaptureH, GlassCapture, GlassTint);
        ...
        private void PlayTissueAppear()
        {
            ...
            scale.ScaleX = DisplaySettings.Instance.TissueStartScale;
            scale.ScaleY = DisplaySettings.Instance.TissueStartScale;
            Top = startTop;
        ...
        private void PlayTissueRetract()
        {
            ...
            AnimationManager.Animate(scale, ScaleTransform.ScaleXProperty, DisplaySettings.Instance.TissueStartScale, dur, ease);
            AnimationManager.Animate(scale, ScaleTransform.ScaleYProperty, DisplaySettings.Instance.TissueStartScale, dur, ease);
            ...
            int retractMs = (int)dur.TimeSpan.TotalMilliseconds;
            AnimationManager.DelayedInvoke(retractMs, () =>
            {
        ...
        private void AnimateTo(double baseW, double baseH, int ms)
        {
            ...
            AnimationManager.Animate(this, WidthProperty, toW, dur, MotionToken.EaseOut);
            AnimationManager.Animate(this, HeightProperty, toH, dur, MotionToken.EaseOut);
            AnimationManager.Animate(this, LeftProperty, toLeft, dur, MotionToken.EaseOut);
            AnimationManager.Animate(RootGrid, FrameworkElement.WidthProperty, baseW, dur, MotionToken.EaseOut);
            AnimationManager.Animate(RootGrid, FrameworkElement.HeightProperty, baseH, dur, MotionToken.EaseOut);
        }
        ...
        private void AnimateExpandSequenced()
        {
            ...
            int token = ++_animationToken;
            int expandMs = (int)dur.TimeSpan.TotalMilliseconds;
            AnimationManager.DelayedInvoke(expandMs, () =>
            {
                if (token != _animationToken) return;
                AnimationManager.Animate(this, HeightProperty, toH, MotionToken.Fast, MotionToken.EaseOut);
                AnimationManager.Animate(RootGrid, FrameworkElement.HeightProperty, baseH, MotionToken.Fast, MotionToken.EaseOut);
            });
        }
        ...
        private void PlayHeartbeatAnimation()
        {
            ...
            // 第 1 段：按下
            int pressMs = (int)MotionToken.Instant.TimeSpan.TotalMilliseconds;
            AnimationManager.Animate(scale, ScaleTransform.ScaleXProperty, MotionToken.BouncePressScale, MotionToken.Instant, MotionToken.EaseOut);
            AnimationManager.Animate(scale, ScaleTransform.ScaleYProperty, MotionToken.BouncePressScale, MotionToken.Instant, MotionToken.EaseOut);

            // 第 2 段：过冲
            int bounceMs = 220; // 弹跳过冲行程比标准时长长
            int token1 = ++_animationToken;
            AnimationManager.DelayedInvoke(pressMs, () =>
            {
                if (token1 != _animationToken) return;
                AnimationManager.Animate(scale, ScaleTransform.ScaleXProperty, MotionToken.BounceOvershoot, TimeSpan.FromMilliseconds(bounceMs), MotionToken.EaseOut);
                AnimationManager.Animate(scale, ScaleTransform.ScaleYProperty, MotionToken.BounceOvershoot, TimeSpan.FromMilliseconds(bounceMs), MotionToken.EaseOut);
            });

            // 第 3 段：归位
            int settleMs = 150;
            int token2 = ++_animationToken;
            AnimationManager.DelayedInvoke(pressMs + bounceMs, () =>
            {
                if (token2 != _animationToken) return;
                AnimationManager.Animate(scale, ScaleTransform.ScaleXProperty, MotionToken.BounceRestScale, TimeSpan.FromMilliseconds(settleMs), MotionToken.EaseOut);
                AnimationManager.Animate(scale, ScaleTransform.ScaleYProperty, MotionToken.BounceRestScale, TimeSpan.FromMilliseconds(settleMs), MotionToken.EaseOut);
            });
        }
        ...
        private void PlaySweepLight()
        {
            ...
            var fadeInMs = (int)MotionToken.Instant.TimeSpan.TotalMilliseconds;
            AnimationManager.Animate(sweep, UIElement.OpacityProperty, 1, MotionToken.Instant, MotionToken.EaseIn);

            var stop = _alertView.SweepStop;
            int token1 = ++_animationToken;
            AnimationManager.DelayedInvoke(fadeInMs, () =>
            {
                if (token1 != _animationToken) return;
                AnimationManager.Animate(stop, GradientStop.OffsetProperty, 1.0, MotionToken.Sweep, MotionToken.EaseInOut);
            });

            int sweepMs = (int)MotionToken.Sweep.TimeSpan.TotalMilliseconds;
            int token2 = ++_animationToken;
            AnimationManager.DelayedInvoke(fadeInMs + sweepMs, () =>
            {
                if (token2 != _animationToken) return;
                AnimationManager.Animate(sweep, UIElement.OpacityProperty, 0, MotionToken.Instant, MotionToken.EaseOut);
            });
        }
        ...
        {
            AnimationManager.Animate(_hoverBorderBrush, System.Windows.Media.Brush.OpacityProperty, 0.20, MotionToken.Fast, MotionToken.EaseOut);
            AnimationManager.Animate(TopHighlight, UIElement.OpacityProperty, 0.65, MotionToken.Fast, MotionToken.EaseOut, onDone);
        }
